...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain1(HttpSecurity http) throws Exception {
        ...
        filter1.setFilterProcessesUrl("/login");
        //filter1.setFilterProcessesUrl("/login2");
      //  http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
        http.csrf().disable();
        http.authorizeHttpRequests().requestMatchers("/login","/","/signupDriver","/abcd").permitAll();
        http.authorizeHttpRequests().requestMatchers("/abc","/lib/**","/assets/**").permitAll();
        ...
        //http.authorizeHttpRequests().requestMatchers("/about/user").hasRole("ADMIN");
